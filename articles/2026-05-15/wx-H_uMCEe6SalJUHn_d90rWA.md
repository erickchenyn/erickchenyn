...
**更进一步看，未来的智能形态将从单智能体向动态生成的集群进化。** Kimi 的 Agent Swarm，OpenClaw 对 Agent Client Protocol (ACP) 支持都指向同一个方向：Agent 从单体应用，演进成分布式、自规划、自组织的软件系统。这要求 Agent Infra 的职责从单一智能体的生命周期管理，升级为对动态协作集群的编排、治理与资源统筹。
...
Agent 的另一项关键要求，是 **云必须更擅长处理"有状态"的任务**。传统云服务更擅长处理无状态、短生命周期的请求，但 Agent 的很多任务需要跨越多轮对话，甚至在暂停一段时间后再继续执行。因此，基础设施要能保留上下文、保存运行现场，并在任务恢复时以接近实时的速度把它重新拉起。对用户而言，这是"记忆连续"；对底层系统而言，则是更复杂的状态管理能力。
