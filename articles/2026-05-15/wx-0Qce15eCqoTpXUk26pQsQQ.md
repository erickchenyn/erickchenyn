...
- 请求被路由至 openYuanrong 函数系统的 Gateway（统一入口），转发至中心调度器，调度器选定空闲节点后完成任务调度。
...
- 指定镜像来源：Docker 镜像 URL（OCI/Nydus），S3、华为 OBS、阿里云 OSS (EROFS image) 均可天然接入

### Checkpoint 和 Restore

可对 Sandbox 进行状态快照与恢复，用于故障恢复或状态保存。（技术细节将在后续技术支柱部分详细展开。）
...
整套 AKernel 集群在拉起的时候就自带了一套完整的监控链路。包括针对每一个节点的资源监控，可全景洞察每个节点的 CPU、Memory 及磁盘 I/O 状态；以及创建一个 Sandbox 的全链路 Trace，精准记录沙箱创建的端到端耗时分布。以具体的例子来看，端到端耗时约 50ms，其中调度阶段耗时约 20+ms，而节点侧沙箱拉起被压缩至 10ms 以内。
...
### 支柱三：Checkpoint / Restore 全链路状态持久化

我们对 openYuanRong 的函数与数据系统进行了深度改造，以支撑全链路的 C/R（Checkpoint/Restore）能力。

用户可通过 SDK 或 CLI 主动对已拉起的 Sandbox 发起 Checkpoint，产出的快照镜像将直接存入 openYuanRong 的数据系统；在用户需要做 Restore 的时候，能够快速从数据系统里将这个 Checkpoint 镜像加载出来，通过它做 Restore。
...
实测的效果，整个部署过程从 23:18:27 开始到 23:27:06 结束，总耗时 8 分 39 秒，完整覆盖了从 VPC 创建、ACK 集群拉起、Node Pool 初始化到最终 Helm 部署的全链路。
